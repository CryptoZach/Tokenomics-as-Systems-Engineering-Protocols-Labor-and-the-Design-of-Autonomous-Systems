#!/usr/bin/env python3
"""
Generate all 19 exhibits for the MeshNet tokenomics whitepaper.
Exhibit 9 (Helium BME timeline) is empirical data, not generated here.
Reads simulation_results.csv and calibration_params.json.
"""
import json
import sys
import numpy as np
...
def exhibit_02():
    ...
    save_exhibit(fig, "exhibit_03_value_flow_comparison.png")
...
def exhibit_03():
    ...
    save_exhibit(fig, "exhibit_04_private_currencies.png")
...
def exhibit_04():
    ...
    add_source(fig, "Source: MeshNet Spec Sheet (Section 4).")
...
def exhibit_07():
    ...
    try:
        ...
        ax_bme.plot(months_real, s2r_df["s2r_clean"], color=COLORS['accent2'],
    ...
    add_source(fig, "Source: MeshNet simulation (top); Helium on-chain data, Dune Analytics (bottom).")
    save_exhibit(fig, "exhibit_10_burn_mint_equilibrium.png")
...
def exhibit_08():
    ...
    fig.tight_layout()
    add_source(fig, "Source: MeshNet Spec Sheet (Section 4.3).")
...
def exhibit_09():
    ...
    fig.tight_layout()
    add_source(fig, "Source: MeshNet Spec Sheet (Section 4.1).")
...
def exhibit_10():
    ...
    add_source(fig, "Source: MeshNet simulation (bull scenario, PID).")
...
def exhibit_11():
    ...
    hide_spines(ax)
    add_source(fig, "Source: Agent-based sell-pressure model (MeshNet simulation).")
...
def exhibit_12():
    ...
    hide_spines(ax)
    add_source(fig, "Source: Conviction voting model (β = 0.05/day).")
...
def exhibit_13():
    ...
    add_source(fig, "Source: MeshNet governance framework (Section 7).")
...
def exhibit_14():
    ...
    add_source(fig, "Source: MeshNet control architecture (Section 5).")
...
def exhibit_15():
    ...
    add_source(fig, "Source: MeshNet simulation (8 configurations).")
...
def exhibit_16():
    ...
    hide_spines(ax)
    add_source(fig, "Source: MeshNet simulation. Dashed = static emission.")
...
def exhibit_18():
    ...
    hide_spines(ax)
    add_source(fig, "Source: MeshNet simulation (OU price model, calibrated from HNT).")
    save_exhibit(fig, "exhibit_21_price_trajectories.png")
...
def exhibit_22():
    ...
    hide_spines(ax)
    add_source(fig, "Source: Monte Carlo simulation (n=100 runs per condition).")
    save_exhibit(fig, "exhibit_25_wash_trading_impact.png")
...
def exhibit_05():
    ...
    hide_spines(ax)
    add_source(fig, "Source: MeshNet simulation; reference lines from real DeFi protocols (Dune Analytics).")
    save_exhibit(fig, "exhibit_06_whale_governance.png")
...
def exhibit_20():
    ...
    for ax, param in zip(axes, ["Kp", "Ki", "Kd"]):
        ...
        ax.text(chosen, ax.get_ylim()[1] * 0.95 if ax.get_ylim()[1] > 0 else 50,
                f' chosen={chosen}',
    ...
    fig.tight_layout()
    add_source(fig, "Source: MeshNet simulation. Each point = 1,825-day run. Other gains held at defaults.")
    save_exhibit(fig, "exhibit_23_pid_sensitivity.png")
...
def exhibit_21():
    ...
    fig.suptitle("Slashing Parameter Sensitivity: Supply Impact Across Scenarios",
                 fontsize=16, fontweight='bold', y=1.02)
    fig.tight_layout()
    add_source(fig, "Source: MeshNet simulation. Each point = 1,825-day run. Other params at defaults.")
    save_exhibit(fig, "exhibit_24_slashing_sensitivity.png")
...
def exhibit_17():
    ...
    for idx, scen in enumerate(scenarios):
        ...
        if scen == 'bull':
            ax.annotate(
                'All 30 seeds \u2192 11,934\n(growth cap: <1 new operator/day, int(30 \u00d7 0.033) = 0)',
    ...
    save_exhibit(fig, "exhibit_20_ensemble_node_distributions.png")
...
def exhibit_19():
    ...
    fig.tight_layout(rect=[0, 0.03, 1, 0.94])
    add_source(fig, "Source: MeshNet 240-run ensemble (30 seeds × 8 configurations). Log scale.")
    save_exhibit(fig, "exhibit_22_ensemble_price_distributions.png")


# NOTE: Exhibits 8 and 9 are produced by separate Helium data scripts:
#   exhibit_08_s2r_timeline.png              — depin-thesis/scripts/build_figure2_s2r.py
#   exhibit_09_bme_12month_decomposition.png — depin-thesis/scripts/helium/bme_12month_analysis.py

ALL_EXHIBITS = [
    ("Exhibit 1: Discipline Map", exhibit_23),
    ("Exhibit 2: Coordination Timeline", exhibit_01),
    ("Exhibit 3: Value Flow Comparison", exhibit_02),
    ("Exhibit 4: Private Currencies", exhibit_03),
    ("Exhibit 5: System Map", exhibit_04),
    ("Exhibit 6: Whale Governance", exhibit_05),
    ("Exhibit 7: Voting Power", exhibit_06),
    # Exhibits 8-9 are external (Helium data charts, not generated here)
    ("Exhibit 10: Burn-Mint Equilibrium", exhibit_07),
    ("Exhibit 11: Dynamic Fee Curves", exhibit_08),
    ("Exhibit 12: Token Allocation", exhibit_09),
    ("Exhibit 13: Emission Schedule", exhibit_10),
    ("Exhibit 14: Airdrop Sensitivity", exhibit_11),
    ("Exhibit 15: Conviction Curves", exhibit_12),
    ("Exhibit 16: Governance Flowchart", exhibit_13),
    ("Exhibit 17: PID Block Diagram", exhibit_14),
    ("Exhibit 18: Emission PID vs Static", exhibit_15),
    ("Exhibit 19: Node Count Stability", exhibit_16),
    ("Exhibit 20: Ensemble Node Distributions", exhibit_17),
    ("Exhibit 21: Price Trajectories", exhibit_18),
    ("Exhibit 22: Ensemble Price Distributions", exhibit_19),
    ("Exhibit 23: PID Gain Sensitivity", exhibit_20),
    ("Exhibit 24: Slashing Sensitivity", exhibit_21),
    ("Exhibit 25: Wash Trading Impact", exhibit_22),
]
...
def main():
    setup_style()
    print("=" * 60)
    print(f"GENERATING {len(ALL_EXHIBITS)} EXHIBITS")
    print("=" * 60)
